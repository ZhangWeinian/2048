...
int game_1::read_number(void)
{
	...
	do
	{
		...
		cout << format("\a输入字符不是数字!请重新输入") << endl;
		cin.clear();
		cin.ignore();
	} while (true);
}

bool game_1::begin(void)
{
	system("cls");
...
void game_1::display_board(void)
{
	system("cls");
	...
	do
	{
		...
		switch (to_where)
		{
			...
			case 100:
				...
				// ESC
			case 27:
				board = vector<vector<int>>(H, vector<int>(H, 0));
				return -1;
